...
class Node:
    ...
    class instead. Node can be sampled by passing the data and error terms (``pd.DataFrame``) to
    ...
    def calculate(self, data: pd.DataFrame, parents: List[str], errors: pd.Series) -> np.ndarray:
        ...
        parents : list(str)
            list of parents, the order of which corresponds to the given param coefficient vector
        errors : numpy array
            sampled errors
...
class Graph:
    ...
    This class creates a causal DAG for the specified nodes and edges. A graph is equipped with
    ...
    dummy_node_prefix : str
        the prefix in the graph description file which identifies dummy nodes: dummy nodes
    ...
    def _set_adj_matrix(self):
        ...
        for n in self.nodes:
            ...
            if self.node_types[n] == 'data' and len(self.parent_sets[n]) != 0:
                raise ValueError(f'node {n} is DataNode but has parents in graph')

    def _single_sample_round(self, node_name: str, data: pd.DataFrame,
    ...
    def _calc_non_interventions(self, node_name: str, data: pd.DataFrame,
                                sampled_errors: pd.DataFrame):
    ...
    def sample(self, size: Optional[int] = None,
        ...
        which then set the size to length of sampled errors. To read more about sampling
        procedure and the meaning of error terms, :ref:`see here <sampling_error_terms>`
        ...
        samples : pd.DataFrame
            If ``return_errors=False``. The return will be a pandas data frame with columns
            equal to node names
            while each row is a sample
        samples, errors : pd.DataFrame, pd.DataFrame
            If ``return_errors=True``. both objects are pandas data frames and similar
